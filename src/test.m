...
% Note: Using the double % creates a section between them and you can use
% Ctrl + Enter to run just that section
...
%% 
% create training examples consisting of 8 second (16 image) windows of 
% data, labeled by whether the subject is viewing a picture or a sentence.  
% Label 1 means they are reading a sentence, label=2 means they are 
% viewing a picture. "examples" is a NxM array, where N is the number of 
% training examples, and M is the number of features per training example 
% (in this case, one feature per voxel at each of 16 time points).

    disp('Begin Training Examples')
...
% train a Naive Bayes classifier
    ...
    disp('Begin Naive Bayes Training')
    [classifier] = trainClassifier(examples, labels, 'nbayes');  % train classifier
    disp('Completed Training')

%% 
% apply the Naive Bayes classifier to the training data (it's best to use 
% cross validation, of course, to obtain an estimate of its true error).  
% The returned array 'predictions' is an array where 
% predictions(k,j) = log P(example_k | class_j).
